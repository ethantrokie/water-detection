...
def plotSpatialFeatures(histMatrix,isWater,numSamples):
    ...
    rects1 = ax.bar(np.arange(0,256), water1, barwidth, color='b', label="Water")
    rects2 = ax.bar(np.arange(0,256) + barwidth, notwater1, barwidth, color='r',label="Not Water")
...
def fourierTransformFullImage(completeVid,boxSize,mask):
    ...
    for div in range(numFrames):
        temporalFeature[:, :, div] = amplitude_spectrum[:, :, div] / sumofSignal
    #plotTimeandFourier(temporalFeature,convolveArr,mask,100,50,202,264)
    return temporalFeature

def plotTimeandFourier(fourierVid,timeVid,mask,xwater,ywater,xnotwater,ynotwater):
